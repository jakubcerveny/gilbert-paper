...
function gilbert2d_bulk_simple() {
  ...
  let cxy = [ two.width/2 + 0,two.height/2 - 35];

  let _h = 280;
  let _w = 280;


  let rect_l  = two.makeRectangle( cxy[0] - _w/4, cxy[1] + _h/4 + _h/16, _w/2, _h/2);
  //rect_l.noFill();
  ...
  let rect_r = two.makeRectangle( cxy[0] + _w/4, cxy[1] + _h/4 + _h/16, _w/2, _h/2);
  //rect_r.noFill();
  ...
  let rect_t = two.makeRectangle( cxy[0]  , cxy[1] - _h/4 + _h/16 , _w, _h/2);
  rect_t.fill = 'rgb(255,255,159)';
  rect_t.linewidth = rect_lw;
  rect_t.stroke = RECT_COLOR;
  rect_t.opacity = 1;
  rect_t.join = "round";


  let _line_v = two.makeLine( cxy[0] - 0.5, cxy[1] + _h/2 + _h/16, cxy[0], cxy[1] + _h/16 );
  let _line_h = two.makeLine( cxy[0] - 0.5 - _w/2, cxy[1] + _h/16, cxy[0] + _w/2, cxy[1] + _h/16 );

  mathjax2twojs("alpha", cxy[0] - 10, cxy[1] + _h/2 + 50, s, 1, true);
  //rbracket( [cxy[0], cxy[1] + _h/2 + 30], 30, _w*0.95, -Math.PI/2 );

  mathjax2twojs("beta", cxy[0] - _w/2-40, cxy[1] + 25, s, 1, true);

  /*

  //rbracket( [cxy[0], cxy[1] + _h/2 + 30], 30, _w*0.95, -Math.PI/2 );
  mathjax2twojs("sigma_abs_beta", cxy[0] - 48, cxy[1] + _h/2 + 60, s, 1, true);

  mathjax2twojs("abs_beta", cxy[0] - _w/2-60, cxy[1] + 6, s, 1, true);
  ...
  */
  ...
  if (show_frame) {
    ...
    let r = two.makeRectangle( two.width/2, two.height/2, two.width, two.height );
    r.noFill();
    r.stroke = "rgb(0,0,0)";
    r.linewidth = 4;
