...
//
function mkg3curve(xy, whd, s, idx_pal) {
  ...
  for (let idx=0; idx<N; idx++) {
    ...
    let __p3 = gilbert.d2xyz(idx, whd[0], whd[1], whd[2]);
    let _p3 = [ __p3.x, __p3.y, __p3.z ];
  ...
  for (let idx=0; idx<n; idx++) {
    ...
    for (let ii=0; ii<draw_lines.length; ii++) {
      ...
      let co = "rgb(32,32,32)";
      co = PAL[0];
      for (let __i=0; __i<idx_pal.length; __i++) {
        if (p_idx < idx_pal[__i]) { break; }
        co = PAL[__i+1];
        //if ((p_idx < idx_pal[__i]) && ((p_idx+1) >= idx_pal[__i])) { break; }
      }

      //let co = hsl_lerp( p_idx / n );
      //if (p_idx < idx0) { co = 'rgb(215,25,28)'; }
      //else { co = 'rgb(13,91,136)'; }
...
function gilbert3d_bulk_example() {
  ...
  let idx_pal = [
    ...
    6*6*6 + 10*4*6,
    6*6*6 + 10*4*6 + 10*6*4,
    6*6*6 + 10*4*6 + 10*6*4 + 10*4*4,
    10*10*10
  ];

  mkg3curve([80,170], [10, 10, 10], 12, idx_pal);
