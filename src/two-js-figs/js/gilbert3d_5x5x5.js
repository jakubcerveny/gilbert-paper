...
function gilbert3d_5x5x5() {
  ...
  for (let i=0; i<(5*5*5); i++) {
    let p = gilbert.d2xyz(i, 5,5,5);
    pnts.push( [p.x, p.y, p.z] );
  }

  //mkg3curve([lx+35,35], [4, 4, 4], 10, {"y":true, "x":false});
  two.makeText("Gilbert:", 50, 20, font_style);
  //pnts = Hibiscus3D(5,5,5);
  mkg3curve([40,170], [5, 5, 5], 30, pnts);

  two.makeText("Gilbert++:", 270, 20, font_style);
  pnts = Gilbert3D(5,5,5);
  mkg3curve([280,170], [5, 5, 5], 30, pnts);

  two.makeText("Hibiscus:", 490, 20, font_style);
  pnts = Hibiscus3D(5,5,5);
  mkg3curve([510,170], [5, 5, 5], 30, pnts);
