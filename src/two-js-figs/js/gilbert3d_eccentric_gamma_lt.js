...
function gilbert3d_eccentric_gamma_lt() {
  let two = g_fig_ctx.two;
  ...
  mk_iso_cuboid( cxy[0]+1.5*dxy[0], cxy[1]+1.5*dxy[1] , 1, lPAL[4], PAL[4], cs, 2, vr, theta );
  mk_iso_cuboid( cxy[0], cxy[1] , 1, lPAL[0], PAL[0], cs, 2, vr, theta );

  let _sm = 0.8*scale;
  let _sl = 1.65*scale;

  mathjax2twojs("one_m_rho", cxy[0]+(2.3*scale), cxy[1]-140, latex_name_scale);
  rbracket( [ cxy[0] + (2.15*scale), cxy[1]-135], _sl/2.65, 0.95*_sl,   theta*0.62);

  mathjax2twojs("rho", cxy[0]+(3.35*scale), cxy[1]-106, latex_name_scale);
  rbracket( [ cxy[0] + (3.20*scale), cxy[1]-100], _sl/3, _sm, theta*0.62);


  // bottom s
  mathjax2twojs("s_d_sigma", cxy[0]-(3.3*scale), cxy[1]-76, latex_name_scale, true);
  rbracket( [cxy[0] - (2.6*scale), cxy[1]-80], _sl/3, 0.95*_sl/2, Math.PI);


  mathjax2twojs("s", cxy[0]-(1.95*scale), cxy[1]-0, latex_name_scale);
  rbracket( [cxy[0] - (1.50*scale), cxy[1]-15], _sl/3, _sl*1.6, Math.PI + theta*0.62);



  rbracket( [cxy[0] + (1.1*scale), cxy[1]+15], _sl/3, 0.9*_sl, - theta*0.86);
  rbracket( [cxy[0] + (2.7*scale), cxy[1]-8], _sl/3, 0.9*_sl, - theta*0.86);

  mathjax2twojs("one_half", cxy[0]+(1.1*scale), cxy[1]+35, latex_name_scale);
  mathjax2twojs("one_half", cxy[0]+(2.7*scale), cxy[1]+12, latex_name_scale);
