...
\begin{abstract}
  ...
  produces paths on 2D rectangles and 3D rectangular cuboids.
  Our construction explicitly manages endpoint parity constraints, which allows us to extend naturally
  from 2D to 3D.
\end{abstract}
...
\begin{itemize}
  \item We describe a simple recursive construction of Hilbert-like paths on arbitrary 2D rectangular grids
  \item We formalize a parity condition on corner endpoints and use it to guide valid recursive subdivisions
...
without any diagonal moves.
We will briefly address the conditions under which the Gilbert curve algorithm introduces diagonal moves throughout this paper.
...
%Many discrete variants exist; 
%the most common recursive discretization applies most directly to
%power-of-two square (2D) and cubic (3D) grids.

%Hilbert defined a space filling curve, which has been since named the \textit{Hilbert curve}, as a continuous map from the unit interval to the unit square \cite{hilbert1891}.
Hilbert defined a continuous, space filling curve that maps the unit interval to the unit square, which as been since named the \textit{Hilbet curve} \cite{hilbert1891}.
One discretization variant applies to square (2D) and cubic (3D) grids with equal sides that are powers of two.
...
%While that approach is 2D-only and not purely uniform across scales,
...
it is also sufficient, and we implicitly use it to choose endpoints and subregions for the recursion.
...
During the course of subdivision, if the cuboid has an aspect ratio of width to either height or depth that exceeds $(3/2)$,
a horizontal bisection is done.
Two other anisotropic cases are tested for when the aspect ratio of height to depth or depth to height exceeds a $(4/3)$ threshold.
The reader is referred to Algorithm \ref{alg:gilbert3d} for further details.

%If subregions have aspect ratio that 
...
one variant of the 3D Hilbert curve.
...
\begin{algorithm}[H]
  ...
  \begin{algorithmic}
    ...
    \If{ $(|\beta| \equiv 1)$ \textbf{ and } $(|\gamma| \equiv 1)$ }
      \State \textbf{yield} $p + i \cdot \delta(\alpha)$ \textbf{forall} $i \in |\alpha|$
    \ElsIf{ $(|\alpha| \equiv 1)$ \textbf{ and } $(|\gamma| \equiv 1)$ }
      \State \textbf{yield} $p + i \cdot \delta(\beta)$ \textbf{forall} $i \in |\beta|$
    \ElsIf{ $(|\alpha| \equiv 1)$ \textbf{ and } $(|\beta| \equiv 1)$ }
      \State \textbf{yield} $p + i \cdot \delta(\gamma)$ \textbf{forall} $i \in |\gamma|$
    \EndIf
...
%First, while our anisotropic-split thresholds are simple and work well in practice, one can define more
...
%Finally, more refined choices of subdivision schemes could be explored that focus on regularity metrics and/or perceptual quality.

While our anisotropic-split thresholds are simple and work well in practice, future work could define more
explicit optimization criteria for when and how to switch subdivision templates.
