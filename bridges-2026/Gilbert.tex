...
%  \item Valid algorithm on arbitrary rectangular regions in 2D and 3D
%  \item Aesthetically pleasing resulting curve
%\end{itemize}
...
Figure \ref{fig:main2dsubdivExample} provides a 2D example of the main subdivision with regions labeled and color coded.
...
Figures \ref{fig:main2dsubdiv} and \ref{fig:gilbert2d_bulk} two different representations of the template used for the
subdivision.
...
Defect reductions for the 3D case can be done, but is more complicated.
...
$(\Delta( \rho_2,\rho ) \overset{\mathrm{def}}{=} \delta(\rho) \textit{ if } |\rho _ 2| \equiv 1 \bmod{2} \text{ and } |\rho| > 2 \textit{ else } 0)$,
...
Each sub-region is an axis-aligned cuboid, allowing for easy calculation of path length as the product of side lengths for
a given volume.
