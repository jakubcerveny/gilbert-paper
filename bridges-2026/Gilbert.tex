...
Figure \ref{fig:main2dsubdiv} shows the 2D template and figure \ref{gilbert3DJSplit} shows the 3D template.
...
When side lengths become too lopsided, an alternate subdivision scheme is chosen to split along
the larger side length or lengths.
...
Here we motivate the eccentric subdivision and the constants chosen for it.

Consider a \textit{defect} function, $\lambda _ d : \mathbb{N}^d \mapsto \mathbb{N}$, that measures the area or volume relative
to what the area or volume would be if it just the minimum side length were taken:
...
We work out the threshold value for the eccentric split of the 2D case.
Threshold values for the 3D case are not provided in this paper.
