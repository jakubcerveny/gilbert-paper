...
Since each sub-region is an axis-aligned cuboid, path length within a region can easily be performed.
...
Spatial position to index lookups can be done by finding the region the position falls in
and then recursively descending with the running index total until the position is encountered.
Both these methods are then $O(\lg(N))$.
...
%Future work could focus on creating a
