...
%
%    \centering
%    \includegraphics[width=0.925\linewidth]{gilbert2d_eccentric_simple.pdf}
%    \caption{ Eccentric subdivision when $|\alpha|$ oblong relative to $|\beta|$ }
...
%    \caption{ Example Gilbert 2D eccentric split when width is oblong relative to height }
%    \label{fig:eccentric_2d_example}
%
%%    \centering
...
\begin{minipage}[ht]{0.45\linewidth}
  ...
  \begin{minipage}[ht]{0.95\linewidth}
    ...
    \includegraphics[width=1.0\linewidth]{gilbert2d_example_eccentric.pdf}
    \captionof{figure}{ }
    \label{fig:eccentric_2d_example_alg1}
...
\begin{minipage}[ht]{0.49\linewidth}
	...
	\begin{minipage}{0.49\textwidth}
		\centering
		\includegraphics[width=\linewidth]{gilbert3d_eccentric_alpha.pdf}
		\label{fig:gilbert3d_eccentric_alpha_zzz}
		\captionof{figure}{ }
	\end{minipage}\hfill
	\begin{minipage}{0.49\textwidth}
		\centering
		\includegraphics[width=\linewidth]{gilbert3d_eccentric_alpha_example.pdf}
		\label{fig:gilbert3d_eccentric_alpha_example_zzz}
		\captionof{figure}{ }
	\end{minipage}
...
After each subdivision, the origion and local axies are rotated to align with the sub
...
%After each subdivision, the origion and local axies are rotated to align with the sub
%region and recursively solved.
%In the base case where there is a unit length side, a line is output.
%If the width like dimension, $\alpha$, is oblong relative to the height-like dimension, $\beta$,
%an eccentric template is used.
%Otherwise, the bulk recursion is executed on the three partioned regions.

Note that even in the 2D case, $\alpha$ and $\beta$ are taken to be vectors in 3D.
%, where the third dimension can be ignored if a purely 2D curve is desired.
...
%Algorithm \ref{alg:gilbert2d} assumes 
...
Each subregion is an axis-aligned rectangle or cuboid, allowing for easy calculation of path length as the product of the side lengths for
...
Generalizing a Hilbert space filling curve to arbitrary side lengths is open to intepretation
...
\begin{thebibliography}{99}
  ...
  \bibitem{anders2009} S. Anders. ``Visualization of genomic data with the Hilbert curve.'' 2009. 10.1093/bioinformatics/btp152.

  \bibitem{zhang2006} J. Zhang, S. Kamata, and Y. Ueshige. ``A Pseudo-hilbert Scan Algorithm for Arbitrarily-Sized Rectangle Region.'' 10.1007/11821045 31.

  \bibitem{cortesi2011} A. Cortesi. ``Visualizing binaries with space-filling curves.'' \url{https://corte.si/posts/visualisation/binvis/}.

  \bibitem{sagan1994} H. Saga. ``Space-Filling Curves.'' 9783540942658.
