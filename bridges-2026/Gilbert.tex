...
%  produces Hamiltonian paths on \emph{even width} 2D rectangles and \emph{even sided} 3D rectangular cuboids.
...
%on \textit{even width} 2D rectangles and \textit{even sided} 3D cuboids.
...
Parity constraints of path endpoints preclude all combinations of side lengths from having a strict Hamiltonian
path, a non self-intersecting path visiting every grid cell exactly once without any diagonal
moves, and we will address the conditions under which this happens later.
