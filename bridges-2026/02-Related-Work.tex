...
Since the Hilbert curve is continuous and space filling, many problems that are higher dimensional
can be reduced to considering a single dimensional case by a clever use of the Hilbert
curve. % to map a one dimensional space to a higher dimensional space.
For example, Wiener noticed that Lebesgue integration could be reduced from
...
The locality property of the Hilbert curve can help in situations where there is a desire to cluster higher dimension
areas from a linear access pattern.
For example, Asano et al. discuss space filling curves and their uses in geometric data structures
to create localized spatial access patterns \cite{asano_1997}.
B{\"o}hm, Perdacher and Plant
show speedups in Cholesky decomposition and the Floyd-Warshall algorithm by using a space
filling curve data access schedule to reduce cache misses \cite{bohm_2021}.
...
Clustering can also help with data visualization by helping to pick out patterns for information that is
formatted in one dimension and embedding it in a higher dimensional space.
For example, Munroe used the locality property to create a stylized visualization
of the internet protocol (IPv4) address space \cite{xkcd_195}.
...
Others have generalized the Hilbert curve, in various ways, to overcome the equal powers of two side length limitation.
Hamilton and Rau-Chaplin explore creating Hilbert curves with \textit{unequal} side lengths in \cite{hamilton_2008} but
require the individual side lengths to still be exact powers of two.
Rong, Zhang and Lin \cite{rong_2021} proposed a generalization of the 2D Hilbert curve to arbitrary rectangular
regions that involves subdividing smaller regions into nearest powers of to areas.
...
\textit{eccentric splits}, that help to create more \textit{harmonious} curve realizations.
...
We further extend Tautenhahn's core ideas on when to use alternate subdivision schemes in the eccentric cases and
extend them to 3d.
Tautenhahn's unpublished reasoning behind the constants used in the 2d eccentric split case are recreated
in Appendix \ref{appendix:eccentric_2d} \footnote{ Through personal communication with the authors, Tautenhahn
kindly provided the reasoning for the constants used in the eccentric split }.

For recursive space filling curves, there is leeway in choosing a subdivision scheme.
...
this is undesirable under certain high performance conditions where memory usage needs to be constant or
where constant run-time bounds matter.
...
Jia et al. provide further enhancements for efficient 3D Hilbert curve encoding and decoding in \cite{jia_2022}.
