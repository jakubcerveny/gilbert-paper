...
\begin{itemize}
  ...
  \item Might have stability or continuity issues, depending on the limiting family
        of Gilbert curves chosen
\end{itemize}
...
a special subdivision is performed to try and make further subdivisions more square or cube-like.
...
The condition of a Hamiltonian path through a cuboid region doesn't address locality.
The Gilbert curve has additional features to make its locality similar to the Hilbert curve.
...
locally connected way \footnote{For example, the \textit{zig-zag} curve that fills a rectangular region
by creating horizontal lines that turn around at the edge boundaries. The zig-zag curve is not space
filling as it isn't locally connected.}.
The local connectivity, or locality, is the feature that allows points clustered in dimension one to
...
If a cuboid region has one side that differs in size by more than a factor of $\sigma$, it is said to be \textit{eccentric}.
For example, if $\sigma=(3/2)$ and $N _ x > (3/2) \text{max}(N_y, N_z)$, the cuboid region is said to be eccentric.

In this paper, we talk about \textit{shape templates} that are cuboid subdivision schemes.
Often there will be some choice as to whether to make the side lengths of the subdivided regions even or odd.
