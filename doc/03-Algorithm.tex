...
In this case, since only a single cuboid absorbs the impossibility of a Hamiltonian path from occurring,
...
For cuboids that violate this condition, there will be a required notch.
A cuboid subdivision with a feasible Hamiltonian path is possible
...
Should the original width-like length be even ($\alpha$ in Figure \ref{fig:main2dsubdiv}), the $B$ block will continue to have 
a valid Hamiltonian path.
If the original width-like dimension is odd, a Hamiltonian path is not possible and there is a forced notch that will appear in the subdivided block $B$.
...
path is not possible, indicated by a red cross, and illustrates how the notch is pushed into the $B$ block
...
obligatory notch is effectively ``pushed'' into the $B$ block.
...
The $(14 \times 14)$ (iv) realization highlights the main subdivision scheme used.
...
An atlas of which $J$-split to choose, what parity each of the subdivided cuboid regions has, and how
...
When $|\alpha| > \frac{5}{3} \max(|\beta|, |\gamma|)$, the $S_0$ template is used to partition the cuboid along the $\alpha$ axis.
In the other cases, when $|\gamma| > \frac{3}{2} \max(|\alpha|, |\beta|)$ or $|\beta| < \frac{2}{3} \min(|\alpha|, |\gamma|)$ the $S_1$
template is used. The $S_2$ template is used when
...
$$
\begin{array}{l}
  \text{defect}(|\alpha|, |\beta|, |\gamma|) \overset{\mathrm{def}}{=} \frac{ |\alpha| \cdot |\beta| \cdot |\gamma| }{ (\min(|\alpha|, |\beta|, |\gamma|))^3 }
\end{array}
$$
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{gilbert2d_pathological.pdf}
  \caption{ Two examples of corner cases and pathological conditions for the Gilbert curve.
            (i) An $(11 \times 2)$ Gilbert curve. The $|\alpha|$ side length of 11 precludes
            a notch free path. For the $((2k+1) \times 2)$ case, the notch shows up as a hook
            at the end. (ii) A $(2 \times 7)$ Gilbert curve. For dimensions $(2 \times k)$, the curve reduces to a snake curve.  }
  \label{fig:gilbert2d_pathological}
\end{figure}
...
See Figure \ref{fig:gilbert2d_pathological} for an example configuration.
