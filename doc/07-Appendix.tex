...
%Calculations in this section will justify what threshold value to pick for when to choose a 3D eccentric split over a J-split.
...
the resulting subdivided regions more cube-like.
That is, we want to choose when and how to make the subdivision and resulting curve more \textit{harmonious}.
...
In what follows, we always commit to splitting the cuboid region by half along the $\alpha$ dimension,
coercing the $A$ volume to a desired parity as needed.
...
%What's left is to decide how eccentric the cuboid should be to determine when to use the $S_0$ split.
%
%Take $\sigma > 1$ and $s = \text{min}(|\beta|,|\gamma|)$, with $\sigma s = |\alpha|$.
%The defect of the original volume is:
%
%$$
%\begin{array}{ll}
%  \lambda(|\alpha|,|\beta|,|\gamma|)  & = \frac{ |\alpha| \cdot |\beta| \cdot |\gamma| }{ (\text{min}(|\alpha|,|\beta|,|\gamma|))^3 } \\
%                  & = \frac{ \sigma s \cdot s \cdot s }{ (\text{min}(\sigma s,s,s))^3 } \\
%                  & = \sigma \\
%\end{array}
%$$
%
%If $|\alpha| > 2 |\beta| = 2s$, then $\text{min}(\frac{|\alpha|}{2},|\beta|,|\gamma|) = s$ and
%we have an average defect of the subdivided regions as $\lambda_s(V_1) = \frac{\sigma}{2}$,
%giving us a reduced defect and making progress.
...
%
%We can get a better ratio by asking what the maximum value of $|\alpha| = \sigma s$ is when there is still an average defect reduction.
%In this case, we restrict $|\alpha| < 2 \text{min}(|\beta|,|\gamma|)$, implying $\text{min}(\frac{|\alpha|}{2},|\beta|,|\gamma|) = \frac{|\alpha|}{2}$.
...
%  \to & 4^{\frac{1}{3}} < \sigma \\ 
%  \to & 4^{\frac{1}{3}} = 1.58740\dots < \sigma \\
%\end{array}
%$$
%
...
%
%For convenience, we've assumed $|\beta| = |\gamma|$ in the above analysis but it's easy to check that the conclusions of the
%above calculations remain unchanged when $|\beta| = \mu |\gamma|$ with $\sigma > \mu \ge 1$.
%
%
%
%%%%%
...
%%%%%
%
%\textbf{UPDATED ARGUMENT:} \textit{(uses $\mu$ for other dimension, tries to be more careful with inequalities)}
...
Call $\lambda_0$ the average defect of the original cuboid regoin and $\lambda_1$ the average defect
...
When the elongated side length is not in the same axis as the start and end point, a simple subdivision won't
yield a defect reduction.
We need to be more careful about when and how to subdivide the cuboid to ensure we make a more harmonious split.

\subsubsection{ $|\gamma| >> |\alpha| \sim |\beta|$ }
...
\subsubsection{ $|\beta| << |\alpha| \sim |\gamma|$ }
...
The constants $\sigma$ and $\rho$ are not optimal in terms of reducing the average defect.
$\sigma$ and $\rho$ were chosen as a compromise between providing simple ratios while
still providing an average defect reduction for both of the cases when $|\gamma|$ large or $|\beta|$ small.
...
Here $|\beta|$ is the eccentric length, surpasing $\sigma = 3/2$ of the minimum
...
\subsubsection{ $|\alpha| << |\beta| \sim |\gamma|$ }
