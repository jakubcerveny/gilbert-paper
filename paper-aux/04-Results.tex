...
Algorithm \ref{alg:bin_and_datacollapse} first calculates the sum of all embedded Euclidean distances, binning by linear curve distance,
and placing them in the array $B$.
The frequency of how many entries are summed in $B$ is stored in the array $M$.
Each sum in $B$ is averaged using the corresponding entry in $M$ to create the average embedded Euclidean distance, $S$.
The function $G$ is calculated by setting the normalized index distance entry to the corresponding average Euclidean distance in $S$ multiplied
by the scaling function $x^{-(1+1/d)}$.
